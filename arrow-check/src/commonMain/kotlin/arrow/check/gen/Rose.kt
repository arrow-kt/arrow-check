...
internal typealias Shrinks<A> = Flow<Rose<A>?>
 ...
 */
public data class Rose<out A>(val res: A, val shrinks: Shrinks<A> = emptyFlow()) {
    /**
     * Map over the rose tree, which changes the result and all shrinks
     */
    public fun <B> map(f: (A) -> B): Rose<B> = Rose(f(res), shrinks.map { it?.map(f) })

    /**
     * Zip two rose trees together, this does parallel shrinking.
     */
    public fun <B, C> zip(other: Rose<B>, f: (A, B) -> C): Rose<C> =
        Rose(
            f(res, other.res),
     ...
     */
    public suspend fun <B> flatMap(f: suspend (A) -> Rose<B>?): Rose<B>? = f(res)?.let { roseB ->
     ...
     */
    public fun <B> filterMap(f: (A) -> B?): Rose<B>? =
 ...
 */
public fun <A> Rose<A>.expand(f: (A) -> Flow<A>): Rose<A> =
