 ...
 */
public data class Report<out A>(
...
public sealed class Progress {
  ...
  public data class Shrinking(val report: FailureSummary) : Progress()
}
...
public sealed class Result {
  ...
  public object GivenUp : Result()
  public data class Failure(val summary: FailureSummary) : Result()
 ...
 */
public data class FailureSummary(
  val usedSize: Size,
...
public sealed class FailureAnnotation {
  public data class Input(val text: () -> Doc<Markup>) : FailureAnnotation()
  public data class Annotation(val text: () -> Doc<Markup>) : FailureAnnotation()
}
...
public data class Summary(
  ...
  public companion object {
    public fun monoid(): Monoid<Summary> = object : Monoid<Summary> {
...
public inline class PropertyCount(public val unPropertyCount: Int)
...
// ------- Pretty printing
internal fun PropertyName?.doc(): Doc<Markup> =
  this?.let { unPropertyName.doc() } ?: "<interactive>".text()

internal fun Report<Progress>.prettyProgress(name: PropertyName?): Doc<Markup> = when (status) {
...
internal fun Report<Result>.prettyResult(name: PropertyName?): Doc<Markup> = when (status) {
...
internal fun Coverage<CoverCount>.prettyPrint(tests: TestCount): Doc<Markup> =
...
internal fun Label<CoverCount>.pretty(tests: TestCount, width: ColumnWidth): Doc<Markup> {
...
internal fun coverageBar(p: CoverPercentage, min: CoverPercentage): Doc<Markup> {
  val barWidth = 20
...
internal fun List<Label<CoverCount>>.width(tests: TestCount): ColumnWidth = foldMap(ColumnWidth.monoid()) {
  it.width(tests)
}

internal fun Label<CoverCount>.width(tests: TestCount): ColumnWidth = ColumnWidth(
...
internal fun CoverPercentage.renderCoverPercentage(): String = "$unCoverPercentage%"

// TODO add reproduce notice
internal fun FailureSummary.pretty(): Doc<Markup> = annotations.prettyAnnotations().ifNotEmpty { vSep() + line() } +
...
internal fun List<FailureAnnotation>.prettyAnnotations(): List<Doc<Markup>> =
...
internal fun TestCount.testCount(): Doc<Nothing> = unTestCount.plural("test".text(), "tests".text())

internal fun ShrinkCount.shrinkCount(): Doc<Nothing> = unShrinkCount.plural("shrink".text(), "shrinks".text())

internal fun DiscardCount.discardCount(): Doc<Nothing> =
...
internal fun Doc<Markup>.render(useColor: UseColor): String = alterAnnotations {
...
public fun Report<Progress>.renderProgress(useColor: UseColor, name: PropertyName?): String =
  prettyProgress(name).render(useColor)

public fun Report<Result>.renderResult(useColor: UseColor, name: PropertyName?): String =
...
internal fun SimpleDoc<Style>.renderMarkup(): String {
